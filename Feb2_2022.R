...
#C:\Users\ralfarosanchez\Dropbox\Working\FIRES_ONTARIO\Goverment
...
opts <- options(
  ...
  "reproducible.useGDAL" = "force", ## one of FALSE, TRUE, "force" # I changed it to load Tave
...
wildfires<- prepInputs(
  ...
  fun = "raster::raster", ## TODO: use terra
  destinationPath = inputDir,
  studyArea = studyArea_ROF,rasterToMatch = LCC2015
)
...
#DatasetAge1_proj<-subset(DatasetAge1_proj,TSLF>29)
modage2 <- bam(log(TSLF) ~
                 ti(total_BA, Tave_sm) +
                 s(total_BA, by = LCC) +
                 s(Tave_sm, by = LCC) +
                 s(LCC,bs='fs')+
                 ...
                 s(coords.x1, coords.x2, bs = "gp", k = 500, m = 2),
...
modage2 <- bam((TSLF) ~
               ...
               data = DatasetAge1_proj, method = "fREML",family=tw, discrete = TRUE
)
...
FigHist
ggsave(file.path(figsDir, "plot_age_pred_vs_obs_hists>85.png"), FigHist)
...
png(file.path(figsDir, "Fig1.png"), width = 1200, height = 600, pointsize = 12)
Fig1
dev.off()
...
colnames(DatasetAge2)[16] <- "PrevAge"
...
png(file.path(figsDir, "Fig23.png"), width = 600, height = 300, pointsize = 12)
Figs23
dev.off()
...
#DatasetAgeROF2 <- na.omit(DatasetAgeROF[,-c(7,8)])

DatasetAgeROF2 <- DatasetAgeROF
str(DatasetAgeROF2)
...
rastersim <- rasterize(
  ...
  field = DatasetAgeROF3[, 9], # vals to fill raster with
  fun = mean
)
